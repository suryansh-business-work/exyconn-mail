...
import { mailboxApi, Mailbox, CreateMailboxPayload } from '../../api/mailboxApi';
...
const MailboxesPage: React.FC = () => {
  ...
  return (
    <Box>
      <AppBreadcrumb items={[{ label: 'Dashboard', href: '/dashboard' }, { label: 'Mailboxes' }]} />
      <Box
        display="flex"
            ...
            <Table>
              ...
              <TableBody>
                {mailboxes.length === 0 ? (
                  ...
                  mailboxes.map((mb) => {
                    const quotaPercent =
                      mb.quota > 0 ? Math.min((mb.usedQuota / mb.quota) * 100, 100) : 0;
                    return (
                      <TableRow key={mb._id} hover>
                        <TableCell sx={{ fontWeight: 600 }}>{mb.email}</TableCell>
                        ...
                        <TableCell>{new Date(mb.createdAt).toLocaleDateString()}</TableCell>
