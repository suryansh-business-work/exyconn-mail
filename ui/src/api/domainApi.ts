...
export const domainApi = {
  ...
  getById: (id: string) =>
    apiClient.get<{ success: boolean; data: { domain: Domain } }>(`/domains/${id}`),
  create: (name: string) =>
    apiClient.post<{ success: boolean; data: { domain: Domain } }>('/domains', {
      name,
    }),
  update: (id: string, data: Partial<Domain>) => apiClient.patch(`/domains/${id}`, data),
  delete: (id: string) => apiClient.delete(`/domains/${id}`),
  verifyDns: (id: string) => apiClient.post(`/domains/${id}/verify-dns`),
  getDnsInstructions: (id: string) => apiClient.get(`/domains/${id}/dns-instructions`),
};
