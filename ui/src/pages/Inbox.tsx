...
export default function Inbox() {
  ...
  return (
    <Layout>
      ...
      <Tabs value={folder} onChange={(_, v) => { setFolder(v); setPage(0); }} sx={{ mb: 2 }}>
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                emails.map((email) => (
                  <TableRow key={email._id} sx={{ bgcolor: email.isRead ? 'inherit' : 'action.hover' }}>
                    <TableCell padding="checkbox"><Checkbox /></TableCell>
                    <TableCell>
                      <IconButton size="small" onClick={() => handleToggleStar(email._id, email.isStarred)}>
                    ...
                    <TableCell>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        {email.subject}
                        {!email.isRead && <Chip label="New" size="small" color="primary" />}
                      </Box>
                    </TableCell>
                    <TableCell>{formatDate(email.createdAt)}</TableCell>
                    <TableCell>
                      <IconButton size="small" color="error" onClick={() => handleDelete(email._id)}>
                        <DeleteIcon />
                      </IconButton>
                    </TableCell>
                  </TableRow>
