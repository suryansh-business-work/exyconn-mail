...
import { Mailbox, MailboxInput, MailboxUpdateInput } from '../../services/mailboxService';
...
const validationSchema = Yup.object({
  ...
  name: Yup.string(),
  quotaLimit: Yup.number().min(0, 'Must be positive').required('Quota limit is required'),
  forwardTo: Yup.string().email('Invalid email'),
  autoReplyMessage: Yup.string(),
});

export const MailboxDialog: FC<MailboxDialogProps> = ({ open, mailbox, onClose, onSubmit }) => {
  ...
  const handleSubmit = async (values: typeof initialValues) => {
    try {
      ...
      const data: MailboxInput | MailboxUpdateInput = {
        ...
        autoReplyMessage: values.autoReply ? values.autoReplyMessage : undefined,
            ...
            <DialogContent>
              ...
              <Grid container spacing={2}>
                ...
                <Grid size={{ xs: 12 }}>
                  <FormControlLabel
                    control={
                      <Switch checked={values.autoReply} onChange={(e) => setFieldValue('autoReply', e.target.checked)} />
                    }
