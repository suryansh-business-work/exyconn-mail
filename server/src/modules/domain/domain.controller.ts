...
import { validateCreateDomainDto, validateUpdateDomainDto } from './domain.validators';
...
export const verifyDomainDns = async (req: Request, res: Response): Promise<void> => {
  try {
...
export const getDnsInstructions = async (req: Request, res: Response): Promise<void> => {
  try {
    const result = await domainService.getDnsInstructions(String(req.params.id));
    res.json({ success: true, data: result });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Failed to get DNS instructions';
    res.status(404).json({ success: false, message });
  }
};
