...
import {
  validateCreateMailboxDto,
  validateUpdateMailboxDto,
} from "./mailbox.validators";
...
export const getMailboxes = async (
  ...
  try {
    ...
    const message =
      error instanceof Error ? error.message : "Failed to fetch mailboxes";
...
export const updateMailbox = async (
  ...
  try {
    ...
    const mailbox = await mailboxService.update(
...
export const suspendMailbox = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    ...
    const message =
      error instanceof Error ? error.message : "Failed to suspend mailbox";
    res.status(404).json({ success: false, message });
  }
};

export const activateMailbox = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    ...
    const message =
      error instanceof Error ? error.message : "Failed to activate mailbox";
    res.status(404).json({ success: false, message });
  }
};
...
export const changeMailboxPassword = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
